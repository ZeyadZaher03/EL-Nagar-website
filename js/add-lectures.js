...
addLecturesForm.addEventListener("submit",(e)=>{
    ...
    const isViemoLink = (str)=>{
        const regex = /^(http:\/\/|https:\/\/)(vimeo\.com)\/([\w\/]+)([\?].*)?$/
        if (regex.test(str)) {
            return true
        }
        else{
    ...
    const checkInput = (InputValue,InputSelector,kind,name)=> {
        if(kind === "string"){
            ...
            else if(!isViemoLink(InputValue)) inputMessageRun(InputSelector, 0 ,"please add a valid video url from Viemo") 
            else if(isViemoLink(InputValue)) {
                inputMessageRun(InputSelector, 1)
                return true
            }
        }
    ...
    const validityArr = [checkInput(videoTitleValue,videoTitleInput,"string","Title"),
